...
Component({
  ...
  properties: {
    ...
    template: {
      ...
      observer: function (newVal) {
        console.warn('template change;; ', newVal, this.template);
    ...
    beauty: {
      ...
      value: 5,
      observer: function (newVal) {
        console.warn('beauty change;; ', newVal, this.data.beauty);
        this.setData({
          beauty: newVal
        });
      }
    ...
    muted: {
      ...
      value: false,
      observer: function (newVal) {
        console.warn('muted change;; ', newVal, this.data.muted);
        this.setData({
          muted: newVal
        });
      }
    ...
    muteStatus: {
      ...
      value: {},
      observer: function(newVal) {
        console.warn('mutestatus change', newVal);
        this.setData({
          muteStatus: newVal
        });
      }
  ...
  data: {
    ...
    winHeight: 0,
    originPushURL: '',
    '1v1Templates': ['1v1bigsmall', '1v1horizontal', 'nv1template', '1v1bigsmallWer', 'nv1templateWer'], // 历史原因： 只保留自己小画面，对方大画面， 并取名为 1v1bigsmall, 实际为 1v1playmain
  ...
  methods: {
    ...
    initLayout(templateName) {
      self = this;
      const erMembers = App.globalData.room.interviewers.map( val => ({userID: val.tid,userName: this.getPersonById(val.tid).name, main: this.getPersonById(val.tid).main }));
      const eeMembers = App.globalData.room.interviewees.map(val => ({ userID: val.tid, userName: this.getPersonById(val.tid).name,}));
      console.log(erMembers)
      console.log(eeMembers)
      switch (templateName) {
        case '1v1horizontal':
        case '1v1bigsmall':
          this.setData({
            maxMembers: 9,
            members: [...erMembers, ...eeMembers],
            erMembers: erMembers,
            eeMembers: eeMembers,
      ...
      // console.log(this.data.erMembers)
      // console.log(this.data.eeMembers)
     ...
     * 退出房间
     */
    exitRoom: function (source) {
      console.log('exit reason', source);
      if (!self.data.pusherContext) {
        self.data.pusherContext = wx.createLivePusherContext('rtcpusher', self);
      }
      ...
      self.setData({
        ...
        eeMembers: [...self.data.members.filter(val => val.userID && val.userID.indexOf('ee_') > -1)],
        erMembers: [...self.data.members.filter(val => val.userID && val.userID.indexOf('er_') > -1)]
      });
    },
        ...
        success: function (res) {
          ...
          self.setData({
            ...
            pushURL: pushUrl + '&tx_time=' + new Date().getTime()
              ...
              console.log('now url is', self.data);
          });
        },
        fail: function (res) {
          self.postErrorEvent(CONSTANT.ROOM.ERROR_REQUEST_ROOM_SIG, '获取推流地址错误');
    ...
    onWebRTCUserListPush: function (msg) {
      console.log('user list push ,', msg);
      if (!msg) {
        return;
      }
      ...
      var pushers = [];
      newUserList && newUserList.forEach((val) => {
        let username = this.getPersonById(val.userid).name;
        let main = this.getPersonById(val.userid).main;
    ...
    onPusherJoin: function (res) {
      console.log('pusher join', res);
      res.pushers.forEach(function (val) {
        var emptyIndex = -1;
        let changeIndex = -1;
        var hasPlay = false;
        self.data.members.some( (member, i) => {
          if(member.userID && member.userID === val.userID) {
            hasPlay = !!member.playerContext;
            if (member.accelerateURL !== val.accelerateURL) {
              member.accelerateURL = val.accelerateURL;
            }
            changeIndex = i;
            return true;
          } else if(!member.userID && changeIndex === -1) {
            changeIndex = i;
            return true;
          }
        });
        ...
        //   }
        // }
        if (!hasPlay && changeIndex != -1) {
          val.loading = false;
          // 如果是1V1 则使用固定的id
          if (self.data['1v1Templates'].indexOf(self.data.template) > -1) {
            if(self.data.erMembers.length === 1 && self.data.eeMembers.length === 1) {
          ...
          console.log(val, self.data.members);
          self.data.members[changeIndex] = {
            ...self.data.members[changeIndex], ...val};
          console.log('now members::', self.data);
        }
      });
    ...
    onPusherQuit: function (res) {
      for (var i = 0; i < self.data.members.length; i++) {
        ...
        if (needDelete) {
          self.data.members[i] = {...self.data.members[i]};
        }
      }
    ...
    delPusher: function (pusher) {
      for (var i = 0; i < self.data.members.length; i++) {
        if (self.data.members[i].userID == pusher.userID) {
          ...
          player && player.stop();
          // pusher.playerContext
          self.data.members[i].accelerateURL = null;
        }
      }
      self.setData({
        members: self.data.members
    ...
    //播放器live-player回调
    onPlay: function (e) {
      console.log('=======onplay', e, self.data.members);
      self.data.members.forEach(function (val) {
        // 如果是1v1 则使用固定的playid
