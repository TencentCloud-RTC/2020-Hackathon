...
Page({
  ...
  enterRoom: function() {
    ...
    //   /room/room?roomID=321&template=1v1&debugMode=false&cloudenv=PRO
  ...
  goRoom: function() {
    const url = `../room/room?roomID=123&template=${this.data.template}&debugMode=${this.data.debugMode}&cloudenv=${this.data.cloudenv}`
    this.tapTime = nowTime
    this.checkDeviceAuthorize().then((result)=>{
      console.log('授权成功', result)
      wx.navigateTo({ url: url })
