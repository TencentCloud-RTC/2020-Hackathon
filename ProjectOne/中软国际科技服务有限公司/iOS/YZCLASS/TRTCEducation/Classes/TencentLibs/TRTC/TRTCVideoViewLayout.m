...
- (void)relayout:(NSArray<UIView *> *)players
{
    ...
    if (players.count == 1) {
        players[0].transform = CGAffineTransformIdentity;
    ...
    switch (self.type) {
        case TC_Float: {
            players[0].transform = CGAffineTransformIdentity;
            players[0].frame = (CGRect){.origin = CGPointZero, .size = self.view.frame.size};
            for (NSInteger i = 1; i < players.count; i++) {
                players[i].transform = CGAffineTransformIdentity;
                CGFloat height = (self.view.height - 10) * 0.5;
                CGFloat width = height * 16/9;
                players[i].frame = CGRectMake(0, height + 10, width, height);
            }
        }
            break;
        case TC_Gird: {
...
- (void)relayoutFullScreen:(NSArray<UIView *> *)players
{
    self.subViews = players;
       if (players == nil)
           return;
       ...
       if (players.count == 1) {
           players[0].transform = CGAffineTransformMakeRotation(M_PI_2);
           players[0].frame = self.view.frame;
           return;
       }
       if (players.count == 0)
           return;
       ...
       switch (self.type) {
           case TC_Float: {
               ...
               for (NSInteger i = 1; i < players.count; i++) {
                   CGFloat width = (self.view.width * 9/16 - 10) * 0.5;
                   CGFloat height = width * 16/9;
                   players[i].frame = CGRectMake(30, 30, height, width);
                   players[i].transform =  CGAffineTransformMakeRotation(M_PI_2);
