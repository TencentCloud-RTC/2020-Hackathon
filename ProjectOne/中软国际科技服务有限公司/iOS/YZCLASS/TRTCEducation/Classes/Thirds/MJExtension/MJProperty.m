...
+ (instancetype)cachedPropertyWithProperty:(objc_property_t)property
{
    ...
    //为MJProperty类对象关联一个值为MJProperty对象的属性property
...
- (void)setProperty:(objc_property_t)property
{
    ...
    // 2.成员类型
    NSString *attrs = @(property_getAttributes(property));//打印格式：T@"NSString",C,N,V_value
    NSUInteger dotLoc = [attrs rangeOfString:@","].location;  //12
    NSString *code = nil;
    NSUInteger loc = 1;
    if (dotLoc == NSNotFound) { // 没有,
        code = [attrs substringFromIndex:loc];
    } else {
        code = [attrs substringWithRange:NSMakeRange(loc, dotLoc - loc)];//@"NSString"
    }
