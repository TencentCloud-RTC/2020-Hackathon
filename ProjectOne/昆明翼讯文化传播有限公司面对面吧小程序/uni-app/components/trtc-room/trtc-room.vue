			...
			<view v-if="template === 'grid'">
				<view data-type="template" data-is="grid" data-attr="pusher, streamList, debug, panelName">
					<view :class="'template-grid ' + (streamList.length < 6 ? 'stream-' + streamList.length : 'stream-5')">
						...
						<view class="student-pos"></view>
						<view class="student-pos" style="position: absolute;z-index:2;">
							<view v-if="!over" class="la-square-jelly-box la-2x">
								<view></view>
								<view></view>
							</view>
							<view v-if="!over" style="margin-top: 30rpx;">
								<text>等待面试者{{parter ? parter.parter_name : ''}}进入，</text>
								<text v-if="!sendNotice" style="color:#b58af1;" @click="setNotice">发送通知</text>
								<text v-else style="color:#b58af1;">已通知</text>
							</view>
							<text v-else>面试已结束，请打分</text>
						</view>			
						<view
							v-for="(item, streamID) in streamList"
							:key="streamID"
							v-if="item.src && (item.hasVideo || item.hasAudio)"
							:class="'view-container player-container ' + (item.isVisible ? '' : 'none') + (item.userID.split('groupid')[1].substr(0,1)==3 ? ' student' : '')"
							:data-userid="item.userID"
							:data-streamtype="item.streamType"
						...
						>
							...
							<view class="operation-bar">
								...
								<!-- 信息 -->
								<view class="tip">
									<text>{{item.userID.split('groupid')[1].substr(0,1)==3 ? '面试者' : '评委'}}</text>
								</view>
							</view>
							<!-- <progress class="volume-progress" :percent="item.volume" stroke-width="4"></progress> -->
						</view>
						<view :class="'view-container pusher-container ' + (pusher.isVisible ? '' : 'none') + (userInfo.groupid==3 ? ' student' : '')">
							<live-pusher
								class="pusher"
								:url="pusher.url"
								:mode="pusher.mode"
							...
							<view class="operation-bar">
								<view class="tip">
									<text>{{userInfo.groupid==3 ? '面试者' : '评委'}}</text>
								</view>
								<view class="bar">
									<view class="btn-normal see-items" @tap="queryParterExams">
										<text>面试题目</text>
									</view>
									<!-- <view class="btn-normal" @tap="switchMemberListPanelFun"><image src="../../static/components/trtc-room/static/list.png"></image></view> -->
									<view class="btn-normal see-items" @tap="switchSettingPanelFun">
										<text>设置</text>
									</view>
									<view v-if='userInfo.groupid!=3' class="btn-normal btn-hangup" @tap="hangUpFun"><image src="../../static/components/trtc-room/static/hangup.png"></image></view>
								</view>
							</view>
						</view>
						...
						<!-- <view class="popup-custom">
							<view class="con">
								<view class="items">
									<view class="list" v-for="(item,index) in items">
										<view class="title">
								...
								<view class="align-center button-box">
									<button @click="cancel" type="primary" style="margin-top:40upx;" class="button-large">关闭</button>
								</view>
							</view>
						</view> -->
						<view :class="'panel setting-panel ' + (panelName === 'setting-panel' ? '' : 'none')">
							<view @tap="handleMaskerClickFun" class="close-btn">X</view>
							<view class="panel-header">视频设置</view>
							<view class="panel-body">
								<scroll-view class="scroll-container" scroll-y="true">
	...
	methods: {
		...
		enterRoom: function(params) {
			return new Promise((resolve, reject) => {
				...
				if(this.config.userID.split('_groupid')[1].substr(0,1) == '3') {
				...
				//开始计时
				console.log('this.config.starttime',this.config.starttime)
				console.log('this.config.endtime',this.config.endtime)
